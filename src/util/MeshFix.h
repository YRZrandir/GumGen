...
void FixSelfIntersection( Poly& m, int max_retry )
{
    ...
    {
        throw std::runtime_error("Failed to fix self intersection: " + std::string(e.what()));
    }
...
{
    using Kernel = typename Polyhedron::Traits::Kernel;
    using Triangle = TTriangle<typename Polyhedron::Vertex::size_type>;
    auto faces = internal::FixRoundingOrder<typename Polyhedron::Traits::Kernel, typename Triangle::size_type>(input_vertices, input_faces);
    ...
    do
    {
        faces = internal::RemoveNonManifold<typename Polyhedron::Traits::Kernel, typename Triangle::size_type>(input_vertices, faces, &nb_removed_faces);
        if(cnt++ >= max_retry)
            break;
    } while (nb_removed_faces != 0);
    if(cnt >= max_retry)
    {
        throw std::runtime_error("Cannot remove non-manifold parts. Try increasing retry times.");
    }
...
)
{
    ...
    FixMesh(vertices, faces, mesh, keep_largest_connected_component, large_cc_threshold, fix_self_intersection, filter_small_holes, max_hole_edges, max_hole_diam, refine, max_retry);
}
